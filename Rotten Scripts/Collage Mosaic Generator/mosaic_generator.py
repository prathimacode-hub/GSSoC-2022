...
def Split(image, size): # function to get row*column list of images
...
def Get(image_dir): # function to get list of image
    ...
    for i in file: # path to the files
        ...
        try:
            ...
            im = Image.open(f) # opening the image
            images.append(im)
...
def Match(i_avg, avgs): # function to get the image index to place the image
    ...
    for i in avgs:
        ...
        if dist < min_dist: # calculate min distance
            min_dist = dist;
            min_ind = ind
            ind+=1
...
def Create(images, dims): # function to create grid for easy fitting images
    ...
    width = max([img.size[0] for img in images]) # maximum width
    height = max([img.size[1] for img in images]) # maximum height
